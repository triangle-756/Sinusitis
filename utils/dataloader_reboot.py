    ...
    def pull_item(self, index):
        ...
        if self.input_type == 'data' and self.output_type == 'mask':
            ...
            for slice_path in tmp:
                ...
                imgs.append(dcmimage)
            img,_ = self.transform(self.phase, image, image)
            return (imgs, img[0]) if len(img)==1 else (imgs, img)
...
class SinusitisDataset2(SinusitisDataset):
    ...
    def detach_ROI(self, img):
        ...
        result = np.where(result_1!=0, 1, result_1)
        return torch.Tensor(result)


    def pull_item(self, index):
        raw_imgs, imgs = super().pull_item(index//2)
        key_ = self.keysliceCNN2(imgs)
        result_img = self.detach_ROI(raw_imgs[key_.index(True)]).reshape(1,512,512)#512x512
        for i in range(len(key_)):
            if key_[i] == True:
                raw_img = self.detach_ROI(raw_imgs[i]).reshape(1,512,512)
                result_img = torch.cat([result_img, raw_img], dim=0)
        for i in range(len(result_img)):
            inf_tuple = np.where(result_img[i]>0)
            x_center = round((np.max(inf_tuple[1])+np.min(inf_tuple[1]))/2)
            y_center = round((np.max(inf_tuple[0])+np.min(inf_tuple[0]))/2)
            result_img[y_center, x_center] = 1
        return result_img, len(key_)
